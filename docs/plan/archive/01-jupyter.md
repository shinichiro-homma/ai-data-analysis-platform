# Jupyter（server + mcp + ai-sync）— アーカイブ

> **完了済み Phase の記録。** 進行中・未着手のタスクは [../01-jupyter.md](../01-jupyter.md) を参照。本文中の詳細計画への参照は `docs/tasks/archive/jupyter/` に読み替えること。
...
| 8.3 | AI編集モード（自動ロック制御） | [x] | ノートブック編集系ツール実行時に自動ロック→AI操作→自動アンロックの一連フローが動作する | handleToolCall ミドルウェアによる自動制御。MCPツールとしての ai_edit_start/ai_edit_end は廃止 |
...
| 23.6 | 軽微な負債の解消 | [x] | DATABASE_URL 未設定時のエラーステータスが実行系・エクスポート系で一致する | §4-6/7: except 握りつぶし・Contents POST 重複・タイムアウトエラー組み立て3重複・`sync_state.py` `_seq_store` 残置エントリの掃除 |
...
| # | タスク | ステータス | E2Eテスト | 備考 |
|---|--------|-----------|-----------|------|
| 24.1 | フロントエンドテスト基盤と純粋関数のユニットテスト | [x] | `scripts/test.sh jupyterlab-ai-sync` がユニットテストを実行して成功する。空文字・スラッシュのみ等の不正パスの正規化と、開いていないノートブックの検索が null を返すケースを含む（不正入力・異常系） | vitest 導入 + path-utils / notebook-finder のテスト。scripts/test.sh・CI への組み込みを含む（§2-5） |
| 24.2 | 受信イベントの境界検証と手書き部分型の解消 | [x] | 必須フィールド欠落・型不一致のイベントを受信しても例外が伝播せず、警告ログを出してスキップする（不正入力・異常系） | I4 違反の解消。notebook-updater の無検証キャストを zod 検証へ、lock-manager の手書き部分型を ISharedNotebook へ（§2-6、監査指摘 3 のフロント側） |
| 24.3 | ロック制御の公式 API 化 | [x] | ロック中はセル編集・実行・カーネル再起動が無効のまま、カーネル中断は有効（AC3/AC6 維持）。`commands.execute` の差し替えが grep で 0 件になる | CommandRegistry.execute モンキーパッチの除去と、keydown 捕捉・コマンドブロックの一元化（§2-1/3） |
| 24.4 | ファイルブラウザカスタマイズの DOM 依存削減 | [x] | シングルクリック展開・ダブルクリック移動が動作し、ファイルの選択・オープンに影響しない（AC5 維持）。想定外の DOM 構造では標準動作にフォールバックし例外を投げない（異常系） | 非公開 DOM クラス名・data 属性依存を公式 API へ寄せる（§2-2） |
| 24.5 | 同期クライアントのライフサイクルと表示不整合の解消 | [x] | プラグイン破棄時に WebSocket が切断される。revert の debounce 中にノートブックを閉じてもタイマー発火がエラーにならない（異常系） | disconnect 未呼び出しリーク（§2-4）、revert タイマーの panel 再解決（監査指摘 5）、ephemeral イベントと revert の順序ちらつき（§2-8・監査指摘 7） |
