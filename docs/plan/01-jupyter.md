...
| # | タスク | ステータス | E2Eテスト | 備考 |
|---|--------|-----------|-----------|------|
| 22.1 | 編集系7ツールの operateCellWithSync 共通化 | [→] | 7ツールの既存ユニットテストが全件成功し、`grep -l 'operateCellWithSync' jupyter-mcp/src/tools/notebook-*.ts` が7件 | I8 違反の解消。Phase 21.3 で postAiEvent 変更が1箇所で済むようになる |
| 22.2 | API レスポンスの zod 境界検証 | [→] | 不正形式の API レスポンスで `RESPONSE_VALIDATION_ERROR` が発生する。統合テストで実レスポンスが検証を通る | I4 違反の解消。本タスクでは operateCell 関連に限定 |
| 22.3 | ツール定義の各ファイルへの分散 | [→] | `wc -l jupyter-mcp/src/tools/index.ts` が 100 行以下。全ツールファイルが `toolEntry` を export | Phase 21.1 で `mutatesNotebook` を各ファイル内で宣言する前提 |
