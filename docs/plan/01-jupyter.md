...
| 19.6 | カーネル中断時の KeyboardInterrupt レスポンス対応 | [→] | notebook_execute_cell / notebook_execute_batch で KeyboardInterrupt 発生時にエラー種別が MCP レスポンスに含まれる（execute_code は対応済み） | jupyter-mcp: 既存ツールの KeyboardInterrupt ハンドリング改善 |
