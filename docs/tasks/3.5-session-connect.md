...
- [ ] `session_connect` ツールが実装されている
- [ ] `notebook_path` を指定して既存セッションに接続できる
- [ ] `kernel_id` を指定して既存セッションに接続できる
- [ ] 接続成功時に `session_id`, `kernel_id`, `notebook_path`, `status`, `connected` が返る
- [ ] 存在しないセッションを指定すると `SESSION_NOT_FOUND` エラーが返る
- [ ] 入力バリデーションが機能する
- [ ] 結合テストが通る
