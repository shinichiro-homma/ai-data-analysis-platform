...
- [ ] `jupyter-server/Dockerfile` が作成されている
- [ ] ベースイメージが `jupyter/scipy-notebook:python-3.11` である
- [ ] `requirements.txt` をコピーして pip install する設定になっている
- [ ] `jupyter-server/requirements.txt`（空ファイル）が作成されている
- [ ] `/home/jovyan/data` と `/home/jovyan/output` ディレクトリが作成される
- [ ] 後続タスクで作成するファイル（config, scripts）の COPY 命令がコメントアウトで記述されている
- [ ] `docker build` コマンドが成功する（WARNING は許容、ERROR は不可）
