...
- [ ] `npm install` で Vitest と関連パッケージがインストールされる
- [ ] `npm run test:integration` で結合テストが実行できる
- [ ] ノートブック作成→セル追加の一連フローがテストで確認できる
- [ ] 複数セル追加とposition指定がテストで確認できる
- [ ] テスト後にテスト用ノートブックがクリーンアップされる
- [ ] テスト結果のレポートが出力される
