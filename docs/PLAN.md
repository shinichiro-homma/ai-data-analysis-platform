...
| 7.2 | MCPリソース実装 | [x] | リソースURIで画像一覧・取得が可能 | resources/list, resources/read |
| 7.3 | get_image_resource ツール実装 | [x] | MCPツールで画像データ（base64）が取得できる | |
| 7.4 | 結合テスト | [x] | グラフ描画→画像取得→内容確認の一連フロー | |
...
| # | タスク | ステータス | E2Eテスト | 備考 |
|---|--------|-----------|-----------|------|
| 8.1 | 単体テスト整備 | [x] | 全ツールのユニットテストが通る | 144テスト成功 |
| 8.2 | MCP Inspector での動作確認 | [x] | 全ツールがInspectorで表示・実行可能 | |
