...
ルート定義（パス・ハンドラの対応）は `extensions/custom_api/handlers.py` の `get_handlers()` が正。各ハンドラの実装は同ディレクトリの `handlers.py` / `cell_handlers.py` / `contents_handlers.py` / `preview_handlers.py` / `session_handlers.py` / `sql_handlers.py` / `workspace_handlers.py` / `ai_events.py` を参照。契約仕様（メソッド・ステータスコード・入出力）は [docs/design/api-contracts.md](../docs/design/api-contracts.md) を参照。
