...
from .contents_handlers import validate_path
...
def _preview_sync(abs_path: Path, file_format: str, head_rows: int) -> dict:
    """CSV/Parquetファイルのプレビューデータを同期的に読み込む（スレッドプールで実行される想定）
    ...
    if not abs_path.exists():
        raise FileNotFoundError(f"File not found: {abs_path.name}")
...
class ContentsPreviewHandler(BaseCustomHandler):
    ...
    async def get(self, path: str):
        ...
        try:
            ...
            result = await loop.run_in_executor(None, _preview_sync, abs_path, file_format, head_rows)
