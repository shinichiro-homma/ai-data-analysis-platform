...
既存テストの所在確認は `scripts/test-map.sh {コンポーネント名}` を使うこと（tests/ を開いて回らない）。
...
（修正の反復はオーケストレーター側の修正ループが担う。失敗ログを抱えて粘らない）。テストコードは変更しないこと。
...
- **テスト FAIL** → `.claude/skills/test-fix-loop/SKILL.md` のループに従って修正する。パラメータ: タスクコンテキスト（Issue 番号・症状・コンポーネント）、完了条件（イシュー詳細の「修正方針」「テスト計画」）、スコープ情報（`git diff --name-only` の変更ファイル・テストファイル・修正ファイル一覧）、テスト実行コマンド（ステップ 6 で記録したもの）、最大修正回数 3、エスカレーション先「ユーザーに報告して停止」。PASS になったらステップ 7a に進む

### 7a. Green フェーズの確認（メインで実行）

**メインで**フルゲート `scripts/test.sh --quiet {コンポーネント名}`（lint + 型チェック + 全テスト）を実行し、バグ再現テストを含む全テスト PASS を確認してください。フルゲートで新たに FAIL したテストも上記の修正ループで扱う（修正回数は通算）。
