...
チェック手順:
1. {コンポーネント名}/src/ のソースコードから、公開ツール・APIエンドポイントを抽出する
2. 以下のドキュメントの一覧表に、ツール名・エンドポイント名が記載されているか確認する:
   - docs/requirements/{コンポーネント名}.md のツール/API一覧
   - docs/overview.md のツール/API一覧表
   - {コンポーネント名}/CLAUDE.md のツール/API一覧
   - README.md（プロジェクトルート）のコンポーネント表・MCPツール一覧

チェック対象: ツール名・エンドポイント名・パラメータ名・型・必須/任意の**構造レベル**の情報のみ。
チェック対象外: デフォルト値・上限値・許可リスト等の定数、description 文言。

### B. 陳腐化検出（ドキュメントにあるがコードにない）

チェック手順:
1. ドキュメントに記載されたツール名・エンドポイント名がコードに存在するか確認する
2. 存在しない場合、以下を区別する:
