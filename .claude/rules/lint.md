...
## jscpd コピペ検出（informational）

CI の `jscpd (informational)` ジョブがコードの重複率を可視化する。不変条件 I8（rule of three）の違反候補を PR 単位で検出するが、CI はブロックしない（`continue-on-error: true`）。

- **informational である理由**: テストコード等の構造的な重複を含むため誤検知が多い。切り分け期間として blocking にしていない
- **赤くなったときの対処**:
  - CI ログで重複箇所を確認し、共通化すべきか判断する
  - テストコードのセットアップ等の構造的重複は許容（共通化すると可読性が下がる場合）
  - プロダクションコードの重複は I8 に従い共通化を検討する
- **設定**: `.jscpd.json` が正。閾値・除外パターン・最小クローンサイズの具体はコードを参照
