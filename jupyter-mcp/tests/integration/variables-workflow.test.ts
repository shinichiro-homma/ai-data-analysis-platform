...
describe('変数管理のワークフローテスト', () => {
  ...
  describe('データ読み込みと変数確認フロー', () => {
    test('データ読み込み → get_variables で DataFrame を確認', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestWorkspaceAndSession('data-load', createdWorkspaceIds, createdSessionIds);
...
df = pd.read_csv(io.StringIO(csv_data))
  ...
  describe('get_dataframe_info の統合テスト', () => {
    test('DataFrame 作成 → get_dataframe_info で詳細情報取得', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestWorkspaceAndSession('df-info', createdWorkspaceIds, createdSessionIds);
...
df = pd.DataFrame({
  ...
  describe('複数変数の追跡テスト', () => {
    test('複数コード実行 → 変数が累積して保持される', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestWorkspaceAndSession('multi-vars', createdWorkspaceIds, createdSessionIds);
    ...
    test('変数の上書き → 新しい値に更新される', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestWorkspaceAndSession('overwrite', createdWorkspaceIds, createdSessionIds);
  ...
  describe('実践的な分析シナリオ', () => {
    test('インポート → データ作成 → 変換 → 集計', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestWorkspaceAndSession('scenario', createdWorkspaceIds, createdSessionIds);
...
describe('get_dataframe_info エラーハンドリング', () => {
  ...
  test('存在しない変数名を指定 → NOT_FOUND エラー', async () => {
    // 1. セッション作成
    const { sessionId } = await createTestWorkspaceAndSession('err-notfound', createdWorkspaceIds, createdSessionIds);
  ...
  test('DataFrame 以外の変数を指定 → INVALID_VARIABLE_TYPE エラー', async () => {
    // 1. セッション作成
    const { sessionId } = await createTestWorkspaceAndSession('err-type', createdWorkspaceIds, createdSessionIds);
