...
describe('コード実行の結合テスト', () => {
  ...
  async function createTestWorkspace(testName: string): Promise<string> {
    const wsResult = await handleToolCall('workspace_create', {
      name: `test-exec-${testName}-${Date.now()}`,
    });
  ...
  /** テスト用セッションを作成するヘルパー */
  async function createTestSession(testName: string): Promise<{ workspaceId: string; sessionId: string }> {
    const workspaceId = await createTestWorkspace(testName);
  ...
  describe('基本実行テスト', () => {
    test('execute_code で print("hello") を実行し stdout に "hello" が返る', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestSession('hello');
    ...
    test.skip('戻り値を持つ式（1 + 1）の result が返る', async () => {
      ...
      // 1. セッション作成
      const { sessionId } = await createTestSession('expr');
  ...
  describe('エラーハンドリングテスト', () => {
    test('1/0 を実行し ZeroDivisionError の情報が返る', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestSession('zerodiv');
    ...
    test('未定義変数のアクセスで NameError の情報が返る', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestSession('nameerr');
  ...
  describe('タイムアウトテスト', () => {
    test('time.sleep(10) を timeout=2 で実行しタイムアウトエラーが返る', async () => {
      // 1. セッション作成
      const { sessionId } = await createTestSession('timeout');
  ...
  describe('ワークフローテスト', () => {
    test('ノートブック作成 → セッション作成 → x = 42 実行 → print(x) で 42 が出力される', async () => {
      ...
      // 0. ワークスペース作成
      const workspaceId = await createTestWorkspace('wf-vars');
    ...
    test('セル追加 → セッション作成 → pandas インポート → DataFrame 作成・表示', async () => {
      ...
      // 0. ワークスペース作成
      const workspaceId = await createTestWorkspace('wf-pandas');
      ...
      const createNotebookData = parseToolCallResult(createNotebookResult);
      expect(createNotebookData.success).toBe(true);
      const notebookPath = createNotebookData.path as string;
