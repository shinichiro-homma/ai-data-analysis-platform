...
describe('セッション管理の結合テスト', () => {
  ...
  async function createTestWorkspace(testName: string): Promise<string> {
    const wsResult = await handleToolCall('workspace_create', {
      name: `test-session-${testName}-${Date.now()}`,
    });
  ...
  test('session_list で作成したセッションが表示される', async () => {
    // 0. ワークスペース作成
    const workspaceId = await createTestWorkspace('list');
  ...
  test('session_delete でセッションが削除される', async () => {
    // 0. ワークスペース作成
    const workspaceId = await createTestWorkspace('delete');
  ...
  test('セッション作成→一覧確認→削除の一連フローが動作する', async () => {
    // 0. ワークスペース作成
    const workspaceId = await createTestWorkspace('flow');
