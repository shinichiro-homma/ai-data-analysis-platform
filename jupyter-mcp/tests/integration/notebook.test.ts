...
describe('ノートブック基本操作の結合テスト', () => {
  ...
  async function createTestWorkspaceAndSession(testName: string): Promise<{ workspaceId: string; sessionId: string }> {
    const wsResult = await handleToolCall('workspace_create', {
      name: `test-notebook-${testName}-${Date.now()}`,
    });
  ...
  test('ノートブックを作成し、code セルを追加できる', async () => {
    ...
    // 0. ワークスペース + セッション作成
    const { workspaceId, sessionId } = await createTestWorkspaceAndSession('code-cell');
    ...
    const notebookPath = createData.path as string;
  ...
  test('ノートブックを作成し、markdown セルを追加できる', async () => {
    ...
    // 0. ワークスペース + セッション作成
    const { workspaceId, sessionId } = await createTestWorkspaceAndSession('md-cell');
  ...
  test('複数のセルを追加し、position 指定で先頭に挿入できる', async () => {
    ...
    // 0. ワークスペース + セッション作成
    const { workspaceId, sessionId } = await createTestWorkspaceAndSession('position');
    ...
    const notebookPath = parseToolCallResult(createResult).path as string;
  ...
  test('ノートブック作成→複数セル追加の一連フローが動作する', async () => {
    ...
    // 0. ワークスペース + セッション作成
    const { workspaceId, sessionId } = await createTestWorkspaceAndSession('flow');
