 ...
 * @returns セッション情報
 */
export function kernelToSessionInfo(kernel: Kernel, includeKernelName = true): SessionInfo {
 ...
 * @returns セッション情報の配列
 */
export function kernelsToSessionList(kernels: Kernel[], includeKernelName = true): SessionInfo[] {
  return kernels.map((kernel) => kernelToSessionInfo(kernel, includeKernelName));
}
