 ...
 */
export async function executeGetDataframeInfo(args: Record<string, unknown>): Promise<McpResponse> {
  const { session_id, variable_name, include_head = true, head_rows = 5 } = args as Partial<GetDataframeInfoArgs>;
  ...
  // 入力検証: variable_name
  const variableNameValidation = validateStringParameter(variable_name, 'variable_name', {
    required: true,
    maxLength: 100,
    allowEmpty: false,
  });
  ...
  if (!variableNameValidation.isValid) {
    return createErrorResponse(variableNameValidation.errorMessage!, 'VALIDATION_ERROR');
  }

  // 入力検証: head_rows
  if (head_rows !== undefined) {
    if (typeof head_rows !== 'number') {
      return createErrorResponse('head_rows パラメータは数値である必要があります', 'VALIDATION_ERROR');
    }

    if (!Number.isInteger(head_rows) || head_rows <= 0) {
      return createErrorResponse('head_rows は正の整数である必要があります', 'VALIDATION_ERROR');
    }

    if (head_rows > 1000) {
      return createErrorResponse('head_rows は最大 1000 行です', 'VALIDATION_ERROR');
    }
  }
  ...
  try {
    ...
    // 変数詳細を取得
    const variable = await jupyterClient.getVariable(kernelId, validatedVariableName);
    ...
    const response: DataFrameInfoResponse = {
      ...
      dtypes: Object.fromEntries(dfVariable.columns.map((col) => [col.name, col.dtype])),
    ...
    if (include_head && dfVariable.head) {
      // head_rows で行数を制限
      const limitedHead = dfVariable.head.slice(0, Math.min(head_rows as number, dfVariable.head.length));
